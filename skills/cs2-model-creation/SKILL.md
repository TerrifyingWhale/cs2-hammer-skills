...
2. **模型路径**：`RenderMeshFile.filename` / `PhysicsMeshFile.filename` 填相对 addon `content/` 根目录的路径（`<文件名>` 换成实际模型名）。
3. **默认材质**：`DefaultMaterialGroup.remaps.to` 填 `materials/<文件名>.vmat`。
...
最基本的模板写在 [references/model-formats.md](references/model-formats.md) 里，生成时直接复制；kv3 头永远一模一样，只替换 `<文件名>` 与路径。

## 生成步骤

1. 确认模型路径（相对 `content/` 根目录）与模型名。
2. 按规则 3/4 确定默认材质。
3. 从 references/model-formats.md 复制模板，替换 `<文件名>` 与路径，写入 `models/` 对应目录；Hammer 自动编译。
